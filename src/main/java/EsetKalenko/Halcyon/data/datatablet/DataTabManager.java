...
public class DataTabManager extends SimpleJsonResourceReloadListener {
    ...
    protected void apply(Map<ResourceLocation, JsonElement> pObject, ResourceManager pResourceManager, ProfilerFiller pProfiler) {
        Entries.tabs.clear();
        Halcyon.LOGGER.info("[HALCYON] Adding Data Tablet Tabs");
        for (Map.Entry<ResourceLocation, JsonElement> i : pObject.entrySet()) {
            ...
            try {
                ...
                Halcyon.LOGGER.info("[HALCYON] Successfully added tab {}", location);
            } catch (IllegalArgumentException | JsonParseException e) {
                Halcyon.LOGGER.error("[HALCYON ERROR] Parsing error loading tab {}", location, e);
            }
        }
        Halcyon.LOGGER.info("[HALCYON] Loaded {} Data Tablet Tabs", Entries.tabs.size());
        ...
        for (DataTab i : toSort) {
            if (i.placement.placeBefore().equals(DataTab.DataTabPlacement.ALL)) {
                if (i.placement.placeAfter().equals(DataTab.DataTabPlacement.ALL)) {
                    Halcyon.LOGGER.error("[HALCYON ERROR] Unable to sort {}, both \"place_after\" and \"place_before\" are set to \"halcyon:all\"", i.id);
                    continue;
                }
                if (!i.placement.placeAfter().equals(DataTab.DataTabPlacement.IGNORED)) {
                    Halcyon.LOGGER.error("[HALCYON ERROR] Unable to sort {}, \"place_before\" is set to \"halcyon:all\" but place_after is not ignored", i.id);
                    continue;
                }
                sorted.addFirst(i);
                ...
                if (!i.placement.placeBefore().equals(DataTab.DataTabPlacement.IGNORED)) {
                    Halcyon.LOGGER.error("[HALCYON ERROR] Unable to sort {}, \"place_after\" is set to \"halcyon:all\" but place_before is not ignored", i.id);
                    continue;
                }
                sorted.addLast(i);
            }
        ...
        for (DataTab i : errored) {
            ...
            if (beforeErrored && afterErrored) {
                ...
                Halcyon.LOGGER.error("[HALCYON ERROR] Unable to sort {}, both \"{}\" and \"{}\" dont exist", i.id, before, after);
            } else {
                String erroredValue = beforeErrored ? i.placement.placeBefore().toString() : afterErrored ? i.placement.placeAfter().toString() : "unknown";
                Halcyon.LOGGER.error("[HALCYON ERROR] Unable to sort {}, \"{}\" doesnt exist", i.id, erroredValue);
            }
        }
        errored.clear();
        ...
        Halcyon.LOGGER.info("[HALCYON] Sorted {} Data Tablet Tabs", Entries.tabsSorted.size());
    }
