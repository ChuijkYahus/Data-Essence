...
public class PingableStructureSerializer {
    public static final MapCodec<PingableStructure> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            ...
            Codec.BOOL.optionalFieldOf("requiresUpgrade", false)
                    .forGetter((data) -> data.requiresUpgrade)
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, PingableStructure> STREAM_CODEC = StreamCodec.of((pBuffer, pValue) -> {
        ...
        pBuffer.writeBoolean(pValue.requiresUpgrade);
    }, pBuffer -> {
        ...
        boolean requiresUpgrade = pBuffer.readBoolean();

        return new PingableStructure(structure, advancement, requiredAdvancement, new Color(color1), new Color(color2), icon, requiresUpgrade);
    });
